...
(function() {
    ...
    async function processPipeline(input) {
        try {
            // JWT tool in pipeline mode: decode JWT token
            let jwt = input.trim();
            ...
            // Return the decoded payload as JSON string for next tool
            return {
                success: true,
    ...
    window.registerCyberSuiteTool({
        id: 'jwt-tool',
        name: 'JWT Decoder',
        description: 'Decode JWT tokens and extract payload',
        icon: 'bi-key-fill',
        category: 'purple',
        
        // Pipeline configuration
        inputTypes: ['jwt', 'text'],  // Accepts JWT strings or plain text
        outputType: 'json',            // Outputs JSON
