...
window.CYBER_RESOURCES_BLUE.push(
  ...
  },
  {
    ...
    "binaries": "https://microsoft.github.io/presidio/installation/",
    "cat": "blue",
    "type": "tool",
    "desc": "PII detection + anonymization SDK (text/images/structured)",
    "details": "Microsoft Presidio is an open-source data protection SDK/framework for detecting and de-identifying sensitive data (PII) across text, images, and structured/semi-structured data. It ships with predefined recognizers (NER + regex + rules) and can be extended with custom recognizers and operators. [oai_citation:0‡microsoft.github.io](https://microsoft.github.io/presidio/)\n\n## Overview\nPresidio is typically used as a **two-step pipeline**:\n1) **Analyze** text to find PII spans/types (Analyzer)\n2) **Anonymize** those spans using operators (replace, mask, redact, encrypt, etc.) (Anonymizer) [oai_citation:1‡microsoft.github.io](https://microsoft.github.io/presidio/analyzer/)\n\n## Setup (API + CLI)\n### API (local Python packages)\nInstall Analyzer + Anonymizer plus an NLP backend (spaCy by default) and a language model: [oai_citation:2‡microsoft.github.io](https://microsoft.github.io/presidio/installation/)\n```bash\npip install presidio_analyzer\npip install presidio_anonymizer\npython -m spacy download en_core_web_lg\n```\n\n### API (HTTP services via Docker)\nPresidio can run as REST services (commonly: **Analyzer on 5002**, **Anonymizer on 5001**), which is convenient for microservices / pipelines. [oai_citation:3‡microsoft.github.io](https://microsoft.github.io/presidio/samples/docker/)\n\nExample API calls:\n```bash\n# Analyze\ncurl -X POST http://localhost:5002/analyze \\\n  -H \"Content-type: application/json\" \\\n  --data '{\"text\":\"John Smith drivers license is AC432223\",\"language\":\"en\"}'\n\n# Anonymize (pass analyzer_results + per-entity operators)\ncurl -X POST http://localhost:5001/anonymize \\\n  -H \"Content-type: application/json\" \\\n  --data '{\n    \"text\":\"hello world, my name is Jane Doe. My number is: 034453334\",\n    \"analyzer_results\":[\n      {\"start\":24,\"end\":32,\"score\":0.8,\"entity_type\":\"NAME\"},\n      {\"start\":48,\"end\":57,\"score\":0.95,\"entity_type\":\"PHONE_NUMBER\"}\n    ],\n    \"anonymizers\":{\n      \"DEFAULT\": {\"type\":\"replace\",\"new_value\":\"ANONYMIZED\"},\n      \"PHONE_NUMBER\": {\"type\":\"mask\",\"masking_char\":\"*\",\"chars_to_mask\":4,\"from_end\":true}\n    }\n  }'\n```\n\n### CLI\nPresidio itself is primarily a Python SDK + optional HTTP services. If you want a **turnkey CLI**, there’s a widely used community wrapper called **presidio-cli** that scans files/directories for PII using Presidio Analyzer (useful in CI and repo scanning). [oai_citation:4‡GitHub](https://github.com/insightsengineering/presidio-cli)\n\n```bash\npython -m pip install presidio-cli\npython -m spacy download en_core_web_sm\n\n# Scan current directory\npresidio .\n\n# Scan with a YAML config (entities/ignore/language)\npresidio -c presidio_cli/conf/limited.yaml tests/\n```\n\n## Practical use\nA common pattern is: **detect → then apply an operator per entity type**.\n\nPython (in-process) example:\n```python\nfrom presidio_analyzer import AnalyzerEngine\nfrom presidio_anonymizer import AnonymizerEngine\nfrom presidio_anonymizer.entities import OperatorConfig\n\ntext = \"My name is Bond, James Bond. Call me at 212-555-5555\"\n\nanalyzer = AnalyzerEngine()\nresults = analyzer.analyze(\n    text=text,\n    language=\"en\",\n    entities=[\"PERSON\", \"PHONE_NUMBER\"],\n)\n\nanonymizer = AnonymizerEngine()\noutput = anonymizer.anonymize(\n    text=text,\n    analyzer_results=results,\n    operators={\n        \"PERSON\": OperatorConfig(\"replace\", {\"new_value\": \"<PERSON>\"}),\n        \"PHONE_NUMBER\": OperatorConfig(\"mask\", {\"masking_char\": \"*\", \"chars_to_mask\": 4, \"from_end\": True}),\n    },\n)\n\nprint(output.text)\n```\nThis directly mirrors the way the Analyzer and Anonymizer are designed to be composed. [oai_citation:5‡microsoft.github.io](https://microsoft.github.io/presidio/analyzer/)\n\n## Notes\nPresidio is an **automated detector** (NER/regex/rules). It can miss things or produce false positives; treat it as one control in a defense-in-depth privacy program, especially for regulated/high-stakes flows. [oai_citation:6‡microsoft.github.io](https://microsoft.github.io/presidio/)\n\nOperational considerations that tend to matter in production:\n- The Analyzer depends on NLP models (e.g., spaCy) and can be tuned/extended with custom recognizers and context logic. [oai_citation:7‡microsoft.github.io](https://microsoft.github.io/presidio/analyzer/)\n- The Anonymizer supports multiple built-in operators and can be extended with custom operators (including reversible workflows via encrypt/decrypt). [oai_citation:8‡microsoft.github.io](https://microsoft.github.io/presidio/anonymizer/)\n- For service deployments, keeping Analyzer+Anonymizer separate lets you scale detection and transformation independently (and keep “raw” data exposure minimized to the detection boundary). [oai_citation:9‡microsoft.github.io](https://microsoft.github.io/presidio/samples/docker/)\n\n## Alternatives\n- **Google Cloud DLP**: managed, very broad detectors (commercial/SaaS).\n- **AWS Comprehend (PII)**: managed PII entity detection (commercial/SaaS).\n- **Azure AI Language (PII)**: managed PII detection, integrates well in Azure ecosystems.\n- **scrubadub** / custom **spaCy NER + regex** pipelines: lighter-weight, but typically less feature-complete than Presidio’s analyzer+operator ecosystem.\n",
    "tags": [
      "pii",
      "privacy",
      "anonymization",
